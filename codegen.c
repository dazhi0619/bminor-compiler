...
void scratch_free(int r) {
  if (r < 0 || r >= 7) {
    printf("Error: failed freeing scratch register %d: Invalid register number\n", r);
    exit(1);
  }
...
const char * scratch_name(int r) {
  if (r < 0 || r >= 7) {
    printf("Error: failed retrieving the name of register %d: Invalid register number\n", r);
    exit(1);
  }
...
const char * arg_reg(int arg) {
  if (0 <= arg && arg <= 5) return arg_name_table[arg];
  else {
...
void expr_codegen(FILE *fp, struct expr *e) {
  ...
  switch (e->kind) {
    ...
    fprintf(fp, "%s:  .string \"%s\"\n", string_label_name(e->string_label), e->string_literal);
    fprintf(fp, ".text\n");
    break;
    ...
    fprintf(fp, "MOVQ %s, %s\n", scratch_name(e->left->reg), arg_reg(arg_count++));
    scratch_free(e->left->reg);
  ...
  switch (d->symbol->kind) {
    ...
    if (d->type->kind == TYPE_FUNCTION) {
        ...
        if (d->value->kind == EXPR_NEG) {
          fprintf(fp, "%s: .quad -%d\n", d->name, d->value->right->literal_value);
