...
int type_equals(struct type *a, struct type *b){
	...
	if(a->kind == b->kind){
        ...
        return false;
			...
			if (!type_equals(a->subtype, b->subtype) || (!a->arr_len || !b->arr_len || a->arr_len->literal_value != b->arr_len->literal_value)) {
...
void type_delete(struct type* t){
	...
	if(t->kind == TYPE_FUNCTION){
		...
		printf("type error: unknown type cannot be deleted. Shouldn't reach here\n");
        TYPECHECK_ERRORNEOUS++;
		return;
	}
...
struct param_list* param_list_copy(struct param_list *p){
	...
	copy = param_list_create(p->name, type_copy(p->type), param_list_copy(p->next));
	return copy;
}
...
struct type * expr_typecheck(struct expr *e){
  ...
  if(e->kind!=EXPR_LIST) lt = expr_typecheck(e->left);
	if(e->kind!=EXPR_CALL && e->kind != EXPR_LIST) rt = expr_typecheck(e->right);
	...
	switch(e->kind){
		...
		case EXPR_GE:
			if(!lt || !rt) break;
			if(!type_equals(lt, rt) || (lt->kind != TYPE_FLOAT && lt->kind != TYPE_INTEGER)){
				printf("type error: Arguments of comparison expression '");
        TYPECHECK_ERRORNEOUS++;
				expr_print(e);
				printf("' must be of type 'integer' or 'float'. Arguments are of type ");
				type_print(lt);
				printf(" and type ");
				type_print(rt);
				printf(" respectively.\n");
        ...
        TYPECHECK_ERRORNEOUS++;
			...
			if(lt->kind != TYPE_INTEGER && lt->kind != TYPE_BOOLEAN && lt->kind != TYPE_FLOAT && lt->kind != TYPE_CHARACTER){
        ...
        TYPECHECK_ERRORNEOUS++;
				expr_print(e);
				printf("' must be of type 'integer' or 'float' at the same time. Arguments are of type ");
				type_print(lt);
				printf(" and type ");
				type_print(rt);
				printf(" respectively.\n");
      ...
      result->arr_len = expr_create_integer_literal(++elem_counter);
			break;
    case EXPR_ARG:
      printf("type eorror: arguments without function call. Shouldn't reach here\n");
      result = type_create(TYPE_VOID, NULL, NULL);
      break;
		default:
...
void decl_typecheck(struct decl *d){
	if(!d) return;
  if(!d->type) { printf("d->type is null. Shouldn't reach here\n"); }
	if(d->type && d->type->kind == TYPE_FUNCTION){
        ...
        TYPECHECK_ERRORNEOUS++;
			...
			printf("type error: unable to determine original return type of function %s. Shouldn't reach here\n", d->name);
        TYPECHECK_ERRORNEOUS++;
		}
	}
    ...
    if(!d->symbol) { printf("d->symbol is null. Shouldn't reach here\n"); }
	...
	switch(s->kind){
      ...
      break;
		case STMT_BLOCK:
			...
			break;
		default:
			printf("type error: unknown statement type detected. Shouln't reach here\n");
        TYPECHECK_ERRORNEOUS++;
			break;
	}
